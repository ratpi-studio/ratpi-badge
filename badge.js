(async function () {
  ...
  try {
    ...
    const latest = tags[0]?.name || "main";

    const validVersion = requestedVersion && tags.some(tag => tag.name === requestedVersion)
      ? requestedVersion
      : latest;

    loadBadgeScript(validVersion);
  } catch (err) {
    console.warn("Erreur GitHub API, fallback sur main");
